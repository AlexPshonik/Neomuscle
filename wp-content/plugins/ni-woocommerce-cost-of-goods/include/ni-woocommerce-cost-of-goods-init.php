...
if( !class_exists( 'Ni_WooCommerce_Cost_Of_Goods_Init' ) ) { 
	class Ni_WooCommerce_Cost_Of_Goods_Init {
		var $ni_constant = array();  
		 ...
		 function admin_menu(){
			add_menu_page( 'Отчёты', 'Отчёты', $this->ni_constant['manage_options'], $this->ni_constant['menu'], array( $this, 'add_page'), 'dashicons-chart-area', "2" );
			add_submenu_page($this->ni_constant["menu"],"Статистика","Статистика", $this->ni_constant['manage_options'],$this->ni_constant["menu"], array( $this, 'add_page'));
			add_submenu_page($this->ni_constant["menu"],"Отчёт о продажах","Отчёт о продажах",  $this->ni_constant['manage_options'],'ni-cog-report', array( $this, 'add_page'));
			
			
				add_submenu_page($this->ni_constant["menu"]
		,__( 'Топ-продукты', 'nisalesreport' )
		,__( 'Топ-продукты', 'nisalesreport' )
		...
		, array(&$this,'add_page'));
		 ...
		 function add_cost_of_goods(){
			 include("ni-woocommerce-cost-of-goods-function.php");

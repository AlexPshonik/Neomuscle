...
class Admin{
	...
	private function registerHooks(){
		...
		add_action('restrict_manage_posts', [ $this, 'productFilterDropdown' ], 999);

		// Bulk edit
		add_action( 'woocommerce_product_bulk_edit_end', [ $this, 'addBrandsToBulkEdit' ] );
		add_action( 'woocommerce_product_bulk_edit_save', [ $this, 'bulkEditBrandsHandler' ] );

		//Settings on permalink page
		add_action( 'admin_init', [ $this, 'addBrandsSettings' ] );
		add_action( 'admin_init', [ $this, 'saveBrandsSettings' ] );

		//duplicate product
		add_action( 'woocommerce_product_duplicate_before_save', [ $this, 'addBrandToDuplicatedProduct' ] , 1 , 2 );
	}
	...
	public function saveBrandsSettings() {
		...
		if( isset( $_POST['permalink_structure'] ) && isset( $_POST['premmerce_brands_base'] ) && $pagenow = 'options-permalink.php'){
			update_option( 'premmerce_brands_base', sanitize_title( $_POST['premmerce_brands_base'] ) );
		}
	}
	...
	public function renderBaseField() {
		?>
			<input type="text" name="premmerce_brands_base" class="regular-text code" value="<?php echo get_option('premmerce_brands_base') ?>" >
		<?php
	}
	...
	public function addBrandsFields(){
		wp_enqueue_media();
		wp_enqueue_style('premmerce-brands', $this->fileManager->locateAsset('admin/css/premmerce-brands.css'), ['woocommerce_admin_styles']);
	...
	public function productFilterDropdown() {
	   ...
	   if ( 'edit.php' === $pagenow && isset($_GET['post_type']) && $_GET['post_type'] == 'product' )
			$this->fileManager->includeTemplate('admin/brands-filter-select.php', [ 'brands' => $brands ] );
	}
	...
	public function addBrandsToBulkEdit() {
		...
		$this->fileManager->includeTemplate('admin/brands-bulk-edit.php', [ 'brands' => $brands ] );
	}

...
class BrandsWidget extends \WP_Widget{
	...
	public function __construct(FileManager $fileManager){
		...
		parent::__construct(
			...
			['description' => __('A list of brands', 'premmerce-brands')]
		);
	}
	...
	public function widget($args, $instance){
		...
		if(!empty($brands) && !is_wp_error($brands)){
			$this->fileManager->includeTemplate('frontend/brands-widget.php', [
				'title'  => $title,
				'args'   => $args,
				'brands' => $brands,
			]);
	...
	public function form($instance){
		...
		$this->fileManager->includeTemplate('admin/brands-widget-form.php', [
			'title'     => $title,
			'mode'      => $mode,
			'onlyPhoto' => $onlyPhoto,
			'limit'     => $limit,
